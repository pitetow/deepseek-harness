...
describe('StatsLine', () => {
  ...
  function props(
    ...
    // The fixture passes no session id, so the cwd/cost seats stay off.
    const unusedSessions = (() => undefined) as never
    return {
      useSession: bindSnapshotSelector(source), useProjection: projections(values),
      useSessions: unusedSessions, sessionId: undefined, t: tEn,
    }
  }
  ...
  it('renders the grouped stats row and hides a brand-new empty session', () => {
    ...
    expect(view.container.textContent).toBe('1 turns · 1 steps| Cache hit 90%| 105 tok total')
  ...
  it('reveals the full line in a delayed hover tooltip only while the row is clipped', () => {
    ...
    expect(view.container.querySelector('[role="tooltip"]')?.textContent)
      .toBe('1 turns · 1 steps | Cache hit 90% | 105 tok total')
  })
  ...
  it('takes every stats label from the active locale', () => {
    ...
    expect(view.container.textContent)
      .toBe('1 轮 · 1 步| LLM 3.8s| 首 token 平均 0.8s · 20 tok/s| 缓存命中 90%| 总 105 tok')
  })
  ...
  it('keeps durable token groups after the visible step window is empty', () => {
    ...
    expect(view.container.textContent)
      .toBe('Cache hit 90%| 105 tok total')
  })
  ...
  it('renders whole-session counts from the sessionStats projection over the paged window', () => {
    ...
    expect(view.container.textContent)
      .toBe('10 turns · 89 steps| Cache hit 90%| 105 tok total')
  })
  ...
  it('keeps the counts group over an empty visible window when the projection carries totals', () => {
    ...
    expect(view.container.textContent)
      .toBe('7 turns · 44 steps| Cache hit 90%| 105 tok total')
  })
  ...
  it('renders whole-log wall times and speeds from the projection, not the loaded window', () => {
    ...
    expect(view.container.textContent).toBe(
      '200 turns · 200 steps| LLM 1m40s · Tool call 1m2s| TTFT avg 0.8s · 20 tok/s| Cache hit 90%| 105 tok total',
    )
  })
  ...
  it('omits cache hit when nothing was billed on the input side', () => {
    ...
    expect(view.container.textContent).toBe('1 turns · 1 steps| 7 tok total')
  })
  ...
  it('includes cache writes in billed input and the cache-hit denominator', () => {
    ...
    expect(view.container.textContent)
      .toBe('1 turns · 1 steps| Cache hit 45%| 207 tok total')
  })
